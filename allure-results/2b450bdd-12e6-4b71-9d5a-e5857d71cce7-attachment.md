...
```ts
  ...
  6  |      console.log(title);
  7  |      const currenturl=await page.url();
  8  |      console.log(currenturl);
  9  | 
  10 |      const msg=await page.locator('input#user-message');
  11 |      console.log(await msg.getAttribute("placeholder"));
  12 |      await page.getByPlaceholder('Please enter your Message').click();
  13 |      await page.getByPlaceholder('Please enter your Message').fill('Vithya');
  14 |      await page.getByRole('button',{name:'Get Checked Value'}).click();
  15 |      const aftermsg=await page.locator('#message').textContent();
  16 |      console.log(aftermsg);
> 17 |      await expect(page.locator('#message')).toHaveText('Vithya');
     |                                             ^ Error: expect(locator).toHaveText(expected) failed
  18 |      await page.close();
  19 | 
  20 | })
