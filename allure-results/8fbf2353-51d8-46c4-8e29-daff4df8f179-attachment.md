...
```ts
  ...
  8  | 
> 9  |   await page.fill('#loginusername',process.env.uname!);
     |              ^ Error: page.fill: value: expected string, got undefined
  10 |   await page.fill('#loginpassword',process.env.pword!);
  11 |   await page.getByRole('button', { name: 'Log in' }).click();
  12 |   await expect(page.locator('#nameofuser')).toContainText('Welcome vithyamurugesan');
  13 | });
```