...
```
Error: locator.textContent: Error: strict mode violation: locator('[class="mt-20"]') resolved to 2 elements:
    1) <p id="message" class="mt-20"></p> aka locator('#message')
    2) <p class="mt-20" id="addmessage"></p> aka locator('#addmessage')

Call log:
  - waiting for locator('[class="mt-20"]')
...
> 16 |      const aftermsg=await page.locator('[class="mt-20"]').textContent();
     |                                                           ^ Error: locator.textContent: Error: strict mode violation: locator('[class="mt-20"]') resolved to 2 elements:
  17 |      console.log(aftermsg);
  18 |      await expect(page.locator('[class="mt-20"]')).toHaveText('Vithya');
  19 | 
  20 | 
  21 | });
  22 | 
