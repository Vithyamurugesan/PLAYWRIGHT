...
- Name: logindemoblaze.test.ts >> test
...
> 11 |   await page.locator('#loginusername').fill(process.env.uname!);
     |                                        ^ Error: locator.fill: value: expected string, got undefined
  12 |   await expect(page.getByRole('img', { name: 'First slide' })).toBeVisible();
  13 | 
  14 |   await page.locator('#loginpassword').click();
  15 |   await page.locator('#loginpassword').fill(process.env.pword!);
  16 |   await page.getByRole('button', { name: 'Log in' }).click();
  17 |   await expect(page.locator('#nameofuser')).toContainText('Welcome vithyamurugesan');
  18 | });
```