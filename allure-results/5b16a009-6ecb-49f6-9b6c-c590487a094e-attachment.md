...
- Name: AUTOMATIONEXERCISE\HerokuFrame.test.ts >> IFrame
- Location: tests\AUTOMATIONEXERCISE\HerokuFrame.test.ts:2:1
...
```ts
  ...
  7  | 
> 8  |     await frame.locator("#tinymce").clear();
     |                                     ^ Error: locator.clear: Error: Element is not an <input>, <textarea>, <select> or [contenteditable] and does not have a role allowing [aria-readonly]
  9  |     await frame.locator("#tinymce").fill("Hello Playwright");
  10 | 
  11 | });
```