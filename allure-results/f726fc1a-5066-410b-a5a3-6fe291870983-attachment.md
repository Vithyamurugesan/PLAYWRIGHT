...
```ts
  ...
  9  | 
> 10 |   await page.fill('#loginusername',process.env.uname!);
     |              ^ Error: page.fill: value: expected string, got undefined
  11 |   await page.fill('#loginpassword',process.env.pword!);
  12 |   await page.getByRole('button', { name: 'Log in' }).click();
  13 |   await expect(page.locator('#nameofuser')).toContainText('Welcome vithyamurugesan');
  14 | });
```