...
- Name: AUTOMATIONEXERCISE\TestMuAiCAlert.test.ts >> Simple Alert
- Location: tests\AUTOMATIONEXERCISE\TestMuAiCAlert.test.ts:3:1
...
Call log:
  ...
  - waiting for locator('#confirm-demo')
    3 × locator resolved to <p class="mt-20" id="confirm-demo"></p>
      - unexpected value ""
...
```ts
  ...
  8  |         console.log(msg);
  9  |         await alert.dismiss();
  10 | 
  11 |     });
  12 |     await page.getByRole("button", { name: "Click Me" }).nth(1).click();
> 13 |     expect(page.locator('#confirm-demo')).toHaveText("You pressed Cancel!");
     |                                           ^ Error: expect(locator).toHaveText(expected) failed
  14 |    
  15 | });
  16 | 
